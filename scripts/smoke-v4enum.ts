...
const stub = (masuk: unknown[], keluar: unknown[], gagal = false) =>
  (async (url: string, init: any) => {
    const b = JSON.parse(String(init?.body ?? '{}'));
    if (b.method !== 'eth_getLogs') return asli(url as any, init);
    if (gagal) return { ok: false, status: 429, json: async () => ({}) };
    const keluarkah = b.params[0].topics[1] === W;
    return { ok: true, status: 200, json: async () => ({ result: keluarkah ? keluar : masuk }) };
...
async function main() {
  ...
  assert.ok(!ids.includes('999001'), 'NFT yang sudah keluar masih terdaftar — urutan peristiwa salah');
  assert.equal(v4ListDegraded(), false, 'scan berhasil tak boleh ditandai degraded');
  ...
  assert.ok(ids.includes('999002'), 'NFT yang kembali tak terdaftar');
  ...
  assert.equal(v4ListDegraded(), true, 'scan gagal harus menandai daftar tak lengkap');
  assert.ok(ps.length > 0, 'scan gagal mengosongkan daftar — posisi bot menghilang');

  globalThis.fetch = asli;
  console.log('ok: urutan kepemilikan benar, kegagalan turun anggun ke v4store');
}
main();
