...
assert(h, 'a real hook must parse');
...
assert(h.powers.some((p) => /cut of every swap/.test(p.label)), 'a swap skim must be detected');
...
assert.equal(mixed?.powers[0].severe, true, 'the severe one belongs at the top');
...
assert.equal(decodeHookFlags('not an address'), null);
