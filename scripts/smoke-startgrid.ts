...
assert.equal(grid.length, 11, `grid harus 11 tombol, terbaca ${grid.length}`);
...
assert.ok(
  ...
  'Add LP / Close LP / Buy / Unwrap tak boleh kembali ke grid — semuanya sudah punya jalur otomatis atau lewat CA',
);
...
console.log('ok: 11 tombol /start punya handler, dan tombol uang dijaga sama seperti command-nya');
